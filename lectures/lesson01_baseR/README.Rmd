...
```{r}
menu <- c(pizza = '$8', lasagna = '$7.50', pasta = '$7')
```
...
2. Create a new branch and switch to it `$ git checkout -b solutions` (`$ git checkout -b [name_of_your_new_branch]` to switch between branches)
...
```{r}
family <- c("Dean", "Sue", "Seth")
```
 
 - Create a vector `birth` with their birth year 
```{r}
birth <- c(1958, 1958, 1992)
```
