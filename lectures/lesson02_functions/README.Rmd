...
avg <- function(x) {
  ...
  if(!is.numeric(x)) stop('`x` is not a number', call. = F)
...
Write a function `yearlySalary` to convert any values <8000 to NA. You can use indexing or a function such as `ifelse()`. Then, output each value as a dollar amount using `scales::dollar()` or `paste()`.
...
yearlySalary <- function(x) {
  ...
  ifelse(x<8000, '$NA', scales::dollar(x))
}
```
