...
iphone5X <- iphoneSales[,c('time','iPhone5C','iPhoneX')]
...
mean(iphoneSales[,'iPhone7'], na.rm = TRUE)

#use inline code to print the mean of the iphone sold in Jun 2018

mean(as.numeric(iphoneSales[iphoneSales$time == 'Jun 2018',]), na.rm = TRUE)
...
iphoneSales_tibble <- as_tibble(iphoneSales)
iphoneSales_tibble[iphoneSales_tibble$time %in% c("Sep 2017", "Dec 2017"), c("iPhone7", "iPhone7Plus")]
...
```{r}
# `$` is a function too. Can you pipe `iphoneSales` to `$` to extract the vector `iPhoneX`? Then, calculate the mean removing NAs
