...
ggplot() +
  ...
  geom_line(data = sn_users, aes(x = year, y = facebook))
```
...
```{r}
myBirthYear <- ifelse(beerDt$Year == 1992, 'my birth year', 'NA')
```
...
```{r}
ggplot() +
  geom_point(data = beerDt, aes(x = Year, y = GallonsCapita, color = myBirthYear))
```
