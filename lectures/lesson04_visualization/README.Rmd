...
ggplot() +
  geom_line(data = twitter_users, aes(x = Year, y = millions)) +
  geom_line(data = fbMobile_users, aes(x = Year, y = millions))
```
...
```{r}
myBirthYear <- ifelse(Year = 1992, 'my birth year', 'NA')
```
...
```{r}
ggplot() +
  geom_point(data = beerDt, aes(x = Year, y = GallonsCapita, fill = myBirthYear))
```
