---
title: 'lesson 5: Data Visualization'
...
ggplot(data = dt) +
  geom_bar(aes(x = College, fill = Sex))

```


Read the documentation for `?geom_bar` to see how you can map `y = ..prop..` instead of `y = ..count..` to plot the proportions of the students in each college.

```{r}
#' 3.
ggplot(data  = dt, aes(x = College, y = ..prop.., group = 1)) +
  geom_bar()
```
...
ggplot(data = dt) +
  geom_bar(aes(x = College, y = ..prop.., group = Sex, fill = Sex), position = 'fill')

#' to make sure your interpretation of the data is consistent
ggplot(data = dt) +
  geom_bar(aes(x = College, y = ..prop.., group = Sex, fill = Sex), position = 'dodge')
```
...
```{r}
ggplot(data = dt) +
  geom_bar(aes(x = College)) +       #plot a count of students in each college
...
ggplot(data = dt) +
  ...
  facet_wrap(~ College) +
  labs(title = "Ethnicity Distribution in Each College", subtitle = "Seth",  caption = "data from www.lsu.edu")
```
...
To select colors, you can use rgb, hex or [color names]((http://www.stat.columbia.edu/~tzheng/files/Rcolor.pdf)

```{r}
ggplot(data = dt) +
  geom_bar(aes(College, fill = Sex)) +
  scale_fill_manual(values  = c('blue', 'navyblue'))
...
ggplot(data = dt) +
  ...
  scale_fill_discrete(name = 'College', labels = 'Female') 
