...
ggplot() +
  stat_count(data = selfiesCasualties, aes(x = class, fill = gender)) +
  geom_bar()

#2. Plot proportions instead of counts for both the previous charts
ggplot() +
  geom_col(data = freqCasualties, aes(x = class, y = n/sum(n), fill = gender))

ggplot() +
  stat_identity(data = selfiesCasualties, aes(x = class, y = prop, fill = gender)) + 
  geom_bar()
...
To select colors, you can sue rgb, hex or [color names]((http://www.stat.columbia.edu/~tzheng/files/Rcolor.pdf)
