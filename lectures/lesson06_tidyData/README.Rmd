...
```{r}
ggplot() +
  geom_line(aes(x  = year, y =  twitter), data = sn_users) +
  geom_line(aes(x = year, y = facebook), data = sn_users)
```
...
```{r}
gather(data = sn_users, key = 'socialNet', value = 'Mil.Users', twitter, facebook) %>% 
ggplot() +
...
gather(data = untidyReview, key = 'review', value = 'value', -reviewId, -reviewStayYear)


#pass the tidy dataset to ggplot() for plotting
gather(data = untidyReview, key = 'review', value = 'reviewValue', -reviewId, -reviewStayYear) %>%
ggplot() +
  geom_bar(aes(x = reviewValue)) + facet_wrap(~review)
```
...
gather(data = iphoneSales, key = 'Quarter', value = 'milSales', -year, na.rm = TRUE) %>% 
  ggplot() +
  geom_line(aes(x= Quarter, y = milSales, color = factor(year), group = year))
```
...
```{r}
#spread() to untidy
spread(data = socialNetwork_users, key = 'company', value = 'percent')
...
spread(data = socialNetwork_users, key = 'company', value = 'percent') %>%
  ggplot() +
  geom_line(aes(x = Year, y = Facebook, color = 'Facebook')) +
  geom_line(aes(x = Year, y = Instagram, color = 'Instagram')) +
  geom_line(aes(x = Year, y = LinkedIn, color = 'LinkedIn')) +
  geom_line(aes(x = Year, y = Pinterest, color = 'Pinterest')) + 
  geom_line(aes(x = Year, y = Twitter, color = 'Twitter'))
```
...
gather(iphoneSales, 'phone', 'sales', -time, na.rm = T) %>%
  ggplot() +
  geom_bar(aes(x = phone, y = sales), stat = 'identity')
```
