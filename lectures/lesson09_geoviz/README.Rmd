...
Not bad, let's do some final tweaks. For instance, it is unclear from the legend which type of variable we are plotting. Since premiums are in dollars, we can add a dollar sign. Also, we override the default labels to include the highest and lowest value on the color-gradient legend:
...
dt <- gather(dt, beverageType, count, -country)

left_join(dt, world, by = c('country' = 'region')) %>%
ggplot() +
  geom_polygon(aes(x = long, y = lat, group = group, fill = count), color = 'white') +
  coord_fixed(1.3) +
  facet_wrap(~beverageType)
```
