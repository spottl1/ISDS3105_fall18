...
Dates are essentially numbers indicating the days from a certain origin (default to `1970-01-01`):
```{r}
typeof(myDate) 
...
dt %>%
  group_by(wday(as.Date(offensedate))) %>%
  summarise(tot = n()) %>%
  arrange(desc(tot))
```
...
dt %>%
  mutate(offensetime = case_when(offensetime_discrete == 'night' ~ '>=10:00pm but 5:00am' ~ night, offensetime_discrete == 'morning' ~ '>5:00am but 10:30am', offensetime_discrete == 'day' ~ '>10:30am but <2:00pm', offensetime_discrete == 'afternoon' ~ '>=2:00pm but < 5:30pm', T ~ 'evening'))
...
dt$iyear <- as.POSIXct.Date(dt$iyear, '%Y')

dt %>%
  select(iyear, France, Italy) %>%
  gather(country, victims, France, Italy) %>%
  ggplot() +
  geom_line(aes(x = iyear, y = victims, color = country)) +
  xlab('date') +
