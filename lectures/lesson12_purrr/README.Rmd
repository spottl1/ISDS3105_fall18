...
bad_driversMountain <- bad_drivers %>%
  mutate(isMountain = case_when(state %in% mountainStates ~ 'Mountain', 
  state %in% c('Louisiana', 'Mississippi', 'Alabama', 'Georgia', 'South Carolina') ~ 'South', T ~ 'Other'), 
  state = tolower(state))
