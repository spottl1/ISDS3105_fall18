...
drawMap <- function(data, title) {
                   ggplot(data = data) +
                      ...
                      coord_fixed(1.3) +
                      labs(title = paste('Insurance Premiums for ', title)) +
...
drawAVG <- function(data, title) {
  ggplot(data = data) +
    geom_line(aes(x = Month, y = AverageRoomRate)) +
    ggtitle('Average Room Rates for Hotel')
}

group_performance1 <- performance %>%
  group_by(hotel) %>%
  nest()

group_performance2 <- group_performance1 %>%
  mutate(rates = pmap(list(data = .$data, title = .$hotel), .f = drawAVG))

drawAVG(group_performance1$data[[1]])

ggsave('lectures/lesson13_purrr_2/timeLines', plot = last_plot(), device = 'png')
```
