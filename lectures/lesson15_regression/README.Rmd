...
### Exercise
 ...
 - Using `fit2`, calculate a prediction interval for the number of crimes for a day with a max temperature of 84 and no rain, and another day with max temperature 84 and 2 inches of rain.
```{r}
toPredict <- data.frame(TempHigh = c(84,84), Precip = c(0,2))

exPI <- predict(fit2, type = 'response', newdata = toPredict, se.fit = T)[1:2]

data_frame(fitted = exPI$fit, lower = (exPI$fit - 1.96*exPI$se.fit), upper = (exPI$fit + 1.96*exPI$se.fit))
...
```{r}
dt_res2 <- fortify(fit2)
ggplot(dt_res2, aes(sample = .resid)) +
  stat_qq() +
  stat_qq_line() +
  ggtitle('Normal QQ-Plot')
ggplot(dt_res2, aes(x = .fitted, y = .stdresid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = 2) +
  ggtitle('Standardized Residuals vs. Fitted')
ggplot(dt_res2, aes(x = .hat, y = .cooksd)) +
  geom_point() +
